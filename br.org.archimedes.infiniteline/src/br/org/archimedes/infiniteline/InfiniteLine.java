...
public class InfiniteLine extends Element implements Offsetable, Filletable {
	...
	public boolean equals(Object object) {
		...
		if (object != null
				&& InfiniteLine.class.isAssignableFrom(object.getClass())) {
	...
	private boolean equals(InfiniteLine other) {
		...
		try {
			...
			boolean oppositeSense = direction.multiply(-1).equals(
					otherDirection);
	...
	public int hashCode() {
		...
		Vector direction = getDirectionVector();
		if (direction.getX() < 0
				|| (direction.getX() <= Constant.EPSILON && direction.getY() < 0))
	...
	public Rectangle getCreationBoundaryRectangle() {
		...
		return new Rectangle(Math.min(initialPoint.getX(), endingPoint.getX()),
				Math.min(initialPoint.getY(), endingPoint.getY()), Math.max(
						...
						initialPoint.getY(), endingPoint.getY()));
