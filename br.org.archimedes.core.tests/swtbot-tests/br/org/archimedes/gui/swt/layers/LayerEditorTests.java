...
public class LayerEditorTests extends SWTBotGefTestCase {
	...
	public void testLineThicknessChange() throws NoActiveDrawingException {
		...
		bot.button("Ok").click();
		Assert.assertEquals(7.536, Utils.getController().getActiveDrawing()
				.getCurrentLayer().getThickness());
	}

	@Test
	public void testDoACircleMoveItUndoAndRedo()
			throws NoActiveDrawingException, InterruptedException,
		...
		Circle o = (Circle) Utils.getController().getActiveDrawing()
				.getCurrentLayer().getElements().toArray()[0];
	...
	@Test
	public void testDoARectangleSelectAllCopyAndPaste()
			throws NoActiveDrawingException, InterruptedException,
		...
		Thread.sleep(1000);
		Polyline o = (Polyline) Utils.getController().getActiveDrawing()
				.getCurrentLayer().getElements().toArray()[0];
	...
	public void testErase() throws InterruptedException,
			NoActiveDrawingException, NullArgumentException,
			InvalidArgumentException {
		bot.menu("Create").menu("Infinite line").click();
		bot.text().setText("0,0;0,0").pressShortcut(Keystrokes.LF);
		bot.text().setText("50,0;50,0").pressShortcut(Keystrokes.LF);
		Thread.sleep(1000);
		InfiniteLine o = (InfiniteLine) Utils.getController()
				.getActiveDrawing().getCurrentLayer().getElements().toArray()[0];
		Assert.assertEquals(o, new InfiniteLine(new Point(0, 0), new Point(50,
				50)));
		...
		Thread.sleep(1000);
		Arc a = (Arc) Utils.getController().getActiveDrawing()
				.getCurrentLayer().getElements().toArray()[1];
		Assert.assertEquals(a, new Arc(new Point(0, 0), new Point(50, 50),
				new Point(50, 0)));
		...
		Assert.assertEquals(1, Utils.getController().getActiveDrawing()
				.getCurrentLayer().getElements().size());
	}
