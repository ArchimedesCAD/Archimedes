...
public class GeometricsTest extends Tester {
	...
	/*
	 * Test method for 'br.org.archimedes.Geometrics.calculateAngle(Point,
	 * Point)' and 'br.org.archimedes.Geometrics.calculateAngle(double, double,
	 * double, double)'
	 */
	@Test
	...
	private void assertExpectedAngle(double x1, double y1, double x2,
		...
		assertEqualsWithinEpsilon(angle,
				Geometrics.calculateAngle(x1, y1, x2, y2));
		try {
	...
	/*
	 * Test method for 'br.org.archimedes.Geometrics.calculateDistance(Point,
	 * Point)' and 'br.org.archimedes.Geometrics.calculateDistance(double,
	...
	private void assertExpectedDistance(double x1, double y1, double x2,
		...
		Assert.assertEquals(distance,
				Geometrics.calculateDistance(x1, y1, x2, y2), Constant.EPSILON);
		try {
	...
	public void testOrthogonalizePointPoint() {
		...
		try {
			Assert.assertTrue("The points should be equals.",
					point3.equals(Geometrics.orthogonalize(point1, point2)));
		} catch (NullArgumentException e) {
	...
	public void testBreakAnglesFromOrigin() {
		...
		Assert.assertEquals(errorMessage, new Point(10, 0),
				Geometrics.breakAngles(p0, new Point(10, 0)));
		Assert.assertEquals(errorMessage, new Point(5, 0),
				Geometrics.breakAngles(p0, new Point(5, 0.525521176)));
		Assert.assertEquals(errorMessage, new Point(5, 0),
				Geometrics.breakAngles(p0, new Point(5, -0.525521176)));
		Assert.assertEquals(errorMessage, new Point(-10, 0),
				Geometrics.breakAngles(p0, new Point(-10, 0)));
		...
		Assert.assertEquals(errorMessage, new Point(-5, 0),
				Geometrics.breakAngles(p0, new Point(-5, -0.525521176)));
		Assert.assertEquals(errorMessage, new Point(0.267949192, -1.0),
				Geometrics.breakAngles(p0, new Point(0.363970234, -1.0)));
		Assert.assertEquals(errorMessage, new Point(0, 5),
				Geometrics.breakAngles(p0, new Point(0.525521176, 5)));
		Assert.assertEquals(errorMessage, new Point(0, 5),
				Geometrics.breakAngles(p0, new Point(-0.525521176, 5)));
		Assert.assertEquals(errorMessage, new Point(6.190658239, 10.722534603),
				Geometrics.breakAngles(p0, new Point(5, 10.722534603)));
		Assert.assertEquals(errorMessage, new Point(0, -5),
				Geometrics.breakAngles(p0, new Point(0.525521176, -5)));
		Assert.assertEquals(errorMessage, new Point(0, -5),
				Geometrics.breakAngles(p0, new Point(-0.525521176, -5)));
		Assert.assertEquals(errorMessage,
				new Point(-6.190658239, 10.722534603),
				Geometrics.breakAngles(p0, new Point(-5, 10.722534603)));
	}
	...
	public void testBreakAnglesOutOfOrigin() {
		String errorMessage = "The point should be equals";
		Point o1 = new Point(1, 1), o2 = new Point(3, 0), o3 = new Point(-1, -1), o4 = new Point(
				-1, 2);

		Point p = Geometrics.breakAngles(o2, new Point(2, 0.305730681));
		Assert.assertEquals(errorMessage, new Point(3, 1),
				Geometrics.breakAngles(o1, new Point(3, 1)));
		Assert.assertEquals(errorMessage, new Point(0, 0),
				Geometrics.breakAngles(o2, new Point(0, 0)));
		Assert.assertEquals(errorMessage, new Point(2, 0.267949192),
				Geometrics.breakAngles(o2, new Point(2, 0.305730681)));
		Assert.assertEquals(errorMessage, new Point(2, -0.267949192),
				Geometrics.breakAngles(o2, new Point(2, -0.305730681)));
		Assert.assertEquals(errorMessage, new Point(4, 0.267949192),
				Geometrics.breakAngles(o2, new Point(4, 0.305730681)));
		Assert.assertEquals(errorMessage, new Point(4, -0.267949192),
				Geometrics.breakAngles(o2, new Point(4, -0.305730681)));
		Assert.assertEquals(errorMessage, new Point(0, -1),
				Geometrics.breakAngles(o3, new Point(0, -1)));
		Assert.assertEquals(errorMessage, new Point(0, 3),
				Geometrics.breakAngles(o4, new Point(0, 3)));

		Assert.assertEquals(errorMessage, new Point(1, 4),
				Geometrics.breakAngles(o1, new Point(1, 4)));
		Assert.assertEquals(errorMessage, new Point(-1, 0),
				Geometrics.breakAngles(o3, new Point(-1, 0)));
		Assert.assertEquals(errorMessage, new Point(-1, 0),
				Geometrics.breakAngles(o4, new Point(-1, 0)));
	...
	public void testCalculateRelativeAngle() {
		...
		Assert.assertEquals(Math.PI / 2.0,
				Geometrics.calculateRelativeAngle(p0, p1, p2), Constant.EPSILON);
		Assert.assertEquals(3.0 * Math.PI / 2.0,
				Geometrics.calculateRelativeAngle(p0, p2, p1), Constant.EPSILON);
		Assert.assertEquals(0.0,
				Geometrics.calculateRelativeAngle(null, null, null),
				Constant.EPSILON);
	}
	...
	public void testCalculateDistancePointLine() throws NullArgumentException {
		...
		Assert.assertEquals(Math.sqrt(2.0),
				Geometrics.calculateDistance(initLine, endLine, point),
				Constant.EPSILON);
		Assert.assertEquals(0.0, Geometrics.calculateDistance(initLine,
				endLine, new Point(0, 0)), Constant.EPSILON);
	}
	...
	public void testGetMeanOfTwoPoints() throws NullArgumentException {
		try {
			Assert.assertEquals(new Point(0, 0),
					Geometrics.getMeanPoint(new Point(0, 0), new Point(0, 0)));
			Assert.assertEquals(new Point(0.5, 0),
					Geometrics.getMeanPoint(new Point(1, 0), new Point(0, 0)));
			Assert.assertEquals(new Point(0, 0),
					Geometrics.getMeanPoint(new Point(-1, -1), new Point(1, 1)));
		} catch (Exception e) {
	...
	public void testCalculateDeterminant() throws NullArgumentException {
		Assert.assertEquals(0.0, Geometrics.calculateDeterminant(
				new Point(1, 2), new Point(2, 2), new Point(3, 2)),
				Constant.EPSILON);
		Assert.assertEquals(9.0, Geometrics.calculateDeterminant(
				new Point(0, 0), new Point(3, 1), new Point(0, 3)),
				Constant.EPSILON);
		Assert.assertEquals(-9.0, Geometrics.calculateDeterminant(new Point(3,
				1), new Point(0, 0), new Point(0, 3)), Constant.EPSILON);
	}

	@Test
	public void testNormalize() {
		double r2 = Math.sqrt(2.0);
		Assert.assertEquals(new Vector(new Point(0, 0), new Point(1, 0)),
				Geometrics.normalize(new Vector(new Point(0, 0),
						new Point(3, 0))));
		Assert.assertEquals(new Vector(new Point(0, 0), new Point(0, 1)),
				Geometrics.normalize(new Vector(new Point(0, 0),
						new Point(0, 3))));
		Assert.assertEquals(new Vector(new Point(0, 0), new Point(1.0 / r2,
				1.0 / r2)), Geometrics.normalize(new Vector(new Point(0, 0),
				new Point(1, 1))));

	}

	@Test
	public void testCalculateArea() throws NullArgumentException,
		...
		Assert.assertEquals(3.0, Geometrics.calculateArea(triangle),
				Constant.EPSILON);
		...
		Assert.assertEquals(4.0, Geometrics.calculateArea(square),
				Constant.EPSILON);
		...
		Assert.assertEquals(4.5, Geometrics.calculateArea(buggie),
				Constant.EPSILON);
		buggie.clear();
		buggie.add(new Point(0, 0));
		buggie.add(new Point(0, 4));
		buggie.add(new Point(3, 0));
		buggie.add(new Point(1, -1));
		Assert.assertEquals(7.5, Geometrics.calculateArea(buggie),
				Constant.EPSILON);
	}

	@Test
	public void testCalculatePerimeter() throws NullArgumentException,
			InvalidArgumentException {
		List<Point> triangle = new ArrayList<Point>();
		triangle.add(new Point(0, 0));
		triangle.add(new Point(2, 0));
		triangle.add(new Point(1, 3));
		Assert.assertEquals(2 * Math.sqrt(10) + 2,
				Geometrics.calculatePerimeter(triangle), Constant.EPSILON);
		List<Point> square = new ArrayList<Point>();
		square.add(new Point(0, 0));
		square.add(new Point(2, 0));
		square.add(new Point(2, 2));
		square.add(new Point(0, 2));
		Assert.assertEquals(8.0, Geometrics.calculatePerimeter(square),
				Constant.EPSILON);
		List<Point> buggie = new ArrayList<Point>();
		buggie.add(new Point(0, 0));
		buggie.add(new Point(0, 4));
		buggie.add(new Point(3, 0));
		buggie.add(new Point(1, 1));
		Assert.assertEquals(9.0 + Math.sqrt(2.0) + Math.sqrt(5.0),
				Geometrics.calculatePerimeter(buggie), Constant.EPSILON);
		buggie.clear();
		buggie.add(new Point(0, 0));
		buggie.add(new Point(0, 4));
		buggie.add(new Point(3, 0));
