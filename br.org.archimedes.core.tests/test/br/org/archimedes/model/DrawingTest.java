...
public class DrawingTest extends Tester {
	...
	public void testPutElement() {
		...
		try {
			...
			Collection<Element> contents = drawing.getUnlockedContents();
			Assert.assertTrue("The element should be in the drawing!",
	...
	public void testSelectionInside() {
		...
		Assert.assertFalse("The line should not be selected",
				sel.contains(line2));
		Assert.assertFalse("The line should not be selected",
				sel.contains(line4));
		Assert.assertFalse("The line should not be selected",
	...
	public void equalsOnlyIfHasTheSameFile() throws Exception {
		...
		assertFalse("A drawing is never equal a random Object",
				drawing.equals(new Object()));

		Drawing other = new Drawing(null);
		assertFalse("A new drawing does not equal a new drawing",
