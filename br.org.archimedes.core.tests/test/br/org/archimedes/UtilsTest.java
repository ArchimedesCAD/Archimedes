...
public class UtilsTest {
	...
	public void testIsPoint() {
		...
		Assert.assertFalse("Should be false for null argument",
				Utils.isPoint(null));
		Assert.assertFalse("Should be false for \"\"", Utils.isPoint(""));
		Assert.assertFalse("Should be false for \"This is not a point.\"",
				Utils.isPoint("This is not a point."));
		Assert.assertTrue("Should be true for \"0;0\"", Utils.isPoint("0;0"));
		Assert.assertTrue("Should be true for \"1.0;1.1\"",
				Utils.isPoint("1.0;1.1"));
		Assert.assertTrue("Should be true for \"1,0;1,0\"",
				Utils.isPoint("1,0;1,0"));
		Assert.assertTrue("Should be true for \"1,0;1.0\"",
				Utils.isPoint("1,0;1.0"));
		Assert.assertTrue("Should be true for \"0 ; 0\"",
				Utils.isPoint("0 ; 0"));
		Assert.assertTrue("Should be true for \"  1.0 ;1.1\"",
				Utils.isPoint("  1.0 ;1.1"));
		Assert.assertTrue("Should be true for \"0 ;0  \"",
	...
	public void testIsDouble() {
		...
		Assert.assertFalse("The string null should return false",
				Utils.isDouble(null));
		...
		Assert.assertTrue("Should be true to \"1.0\"", Utils.isDouble("1.0"));
		Assert.assertTrue("Should be true to \"+1,213\"",
				Utils.isDouble("+1,213"));

		Assert.assertFalse("Should be false to \"mamae\"",
				Utils.isDouble("mamae"));
		Assert.assertFalse("Should be false to \"5.0;6.1\"",
				Utils.isDouble("5.0;6.1"));
	...
	public void testTransformVector() throws NullArgumentException {
		...
		Assert.assertEquals(new Point(1, 1),
				Utils.transformVector(new Point(0, 0), new Point(1, 1)));
		workspace.setShiftDown(true);
		Assert.assertEquals(
				Geometrics.breakAngles(new Point(0, 0), new Point(1, 1)),
				Utils.transformVector(new Point(0, 0), new Point(1, 1)));
	}
}
