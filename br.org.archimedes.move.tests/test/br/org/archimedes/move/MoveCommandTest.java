...
public class MoveCommandTest extends Tester {
	...
	public void testEqualsWhenVectorIsDifferent() throws Exception {
		...
		MoveCommand moveRight = new MoveCommand(pointsToMove, new Vector(
				new Point(1.0, 0.0)));
		MoveCommand anotherMoveRight = new MoveCommand(pointsToMove,
				new Vector(new Point(1.0, 0.0)));
		MoveCommand moveFurtherRight = new MoveCommand(pointsToMove,
				new Vector(new Point(2.0, 0.0)));
		MoveCommand moveUp = new MoveCommand(pointsToMove, new Vector(
				new Point(0.0, 1.0)));
	...
	public void testEqualsWithDifferentPointsToMove() throws Exception {
		...
		otherPointsToMove
				.put(line2, Collections.singleton(new Point(1.0, 1.0)));
		MoveCommand moveFirstLineExtreme = new MoveCommand(pointsToMove,
				directionToMove);
		MoveCommand moveSecondLineExtreme = new MoveCommand(otherPointsToMove,
				directionToMove);
		otherPointsToMove.clear();
		otherPointsToMove.putAll(pointsToMove);
		MoveCommand otherMoveFirstLineExtreme = new MoveCommand(
				otherPointsToMove, directionToMove);
		pointsToMove.clear();
		pointsToMove.put(line1, Collections.singleton(new Point(0.0, 0.0)));
		MoveCommand moveOtherFirstLineExtreme = new MoveCommand(pointsToMove,
				directionToMove);
