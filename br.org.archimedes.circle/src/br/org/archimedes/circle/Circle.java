...
public class Circle extends Element implements Offsetable {
	...
	public Collection<ReferencePoint> getReferencePoints(Rectangle area) {
		...
		try {
			ReferencePoint reference = new CirclePoint(this.getCenter(),
					this.getCenter());
			...
			for (double angle = 0; angle < 360; angle += 90) {
				...
				double x = this.getRadius() * Math.cos(radians)
						+ this.getCenter().getX();
				double y = this.getRadius() * Math.sin(radians)
						+ this.getCenter().getY();
	...
	public Point getProjectionOf(Point point) throws NullArgumentException {
		...
		if (getCenter().equals(point)) {
			return new Point(getCenter().getX() + getRadius(), getCenter()
					.getY());
		}
	...
	/**
	 * Returns the Points that are on the circle and on the line defined by the
	 * given Point and the center of the Circle.
	 * 
	 * @param point
	 *            a Point
	 * @return a Collection with the intersections of the infinite line formed
	 *         by the center of this circle with the point.
	 * @throws NullArgumentException
	 *             If the given argument is null
	 */
	private Collection<Point> getCirclePoints(Point point)
			throws NullArgumentException {
	...
	public Element cloneWithDistance(double distance)
		...
		Circle clone = null;
		try {
			clone = new Circle(this.getCenter().clone(), this.getRadius()
					+ distance);
	...
	public boolean isPositiveDirection(Point point) {
		...
		try {
			if (Geometrics.calculateDistance(this.getCenter(), point) > this
					.getRadius()) {
				isOutside = true;
			}
		} catch (NullArgumentException e) {
