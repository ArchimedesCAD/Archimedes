...
public class Arc extends CurvedShape implements Offsetable {
	...
	/**
	 * Constructor. Always build the arc in the counter clockwise order. Will
	 * NOT use the instances of points passed but clones of them.
	 * 
	 * @param initialPoint
	 *            The initial point
	...
	public Arc(Point initialPoint, Point intermediatePoint, Point endingPoint)
		...
		createInternalRepresentation(initialPoint.clone(),
				intermediatePoint.clone(), endingPoint.clone(),
				Geometrics.getCircumcenter(initialPoint, intermediatePoint,
						endingPoint));
	}

	/**
	 * Constructor. Will NOT use the instances of points passed but clones of
	 * them.
	 * 
	 * @param initialPoint
	 *            The initial point
	...
	/**
	 * Adjusts the arc points so that they are in counter clockwise order. It
	 * DOES NOT change the instances of the points passed but rather move them
	 * to the correct location (this way, we always rely on the same points for
	 * the arc). Bug Fix for Bug ID 2746056.
	 * 
	 * @param initialPoint
	 *            The initial point
	 * @param intermediatePoint
	...
	private void createInternalRepresentation(Point initialPoint,
			Point intermediatePoint, Point endingPoint, Point center)
		...
		Point newCenter = Geometrics.getCircumcenter(initialPoint,
		...
		Point newMidPoint = calculateMidPoint(this.initialPoint,
	...
	public int hashCode() {
		...
		result += ((this.initialPoint == null) ? 0 : this.initialPoint
				.hashCode());
		result = prime
				* result
				+ ((this.intermediatePoint == null) ? 0
						: this.intermediatePoint.hashCode());
		return result;
	}
	 ...
	 * @return true if the arc is defined to the same side as me, false
	 *         otherwise (This does NOT check whether the initial and ending
	 *         match).
	 */
	 ...
	 * @return true if the initial and ending point of the arc match with mine.
	 *         They can match perfectly or be switched. Otherwise, returns
	 *         false.
	 */
	 ...
	 * @return true if initial matches my initialPoint and ending matches my
	 *         endingPoint, false otherwise.
	 */
	private boolean matchInitialAndEnding(Point initial, Point ending) {

		return ((this.initialPoint.equals(initial)) && (this.endingPoint
				.equals(ending)));
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.org.archimedes.model.Element#contains(br.org.archimedes
	 * .model.Point)
	 */
	...
	public boolean contains(Arc arc) throws NullArgumentException {
		...
		return (this.contains(arc.getInitialPoint())
				&& this.contains(arc.getIntermediatePoint()) && this
	...
	/**
	 * This method was overwritten because, although mirror is performed, the
	 * arc still kept unaltered references to the ending and initialPoint.
	 * However, when mirroring, those points are inverted. We have to apply this
	 * change here.
	 * 
