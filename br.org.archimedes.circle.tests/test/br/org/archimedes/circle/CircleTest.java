...
public class CircleTest extends Tester {
	...
	@Test
	public void testProjectionOfAPointOnTheCircle()
			throws NullArgumentException {

		Assert.assertEquals(new Point(0, 1),
				circle1.getProjectionOf(externalPoint));

		Assert.assertEquals(new Point(0, 1),
				circle1.getProjectionOf(internalPoint));

		Assert.assertEquals(new Point(1, 0),
				circle1.getProjectionOf(centerPoint));
	}
	...
	public void testEqualsCircle() throws Exception {
		assertTrue(circle1.equals(new Circle(new Point(0, 0), Double
				.valueOf(CIRCLE_RADIUS))));
	...
	public void testHashCodeCircle() throws Exception {
		...
		assertEquals(hash1, circle1.hashCode()); // Test if it returns the same
													// code again
		...
		c1.move(5, 0);
		assertFalse(hash_c1 == c1.hashCode()); // Object changed, hash should
												// change
	}

	@Test
	public void testBoundaryRectangle() {
		assertEquals(circle1.getBoundaryRectangle(),
	...
	public void testReferencePoint() throws Exception {
		...
		assertCollectionTheSame(refPoints,
				circle1.getReferencePoints(new Rectangle(50, 50, 51, 51))); // Empty
		...
		assertCollectionTheSame(refPoints,
				circle1.getReferencePoints(new Rectangle(0, 100, 100, 0))); // Only
																			// Top-Right

		refPoints.add(new RhombusPoint(new Point(-1, 0)));
		refPoints.add(new RhombusPoint(new Point(0, -1)));
		assertCollectionTheSame(refPoints,
				circle1.getReferencePoints(new Rectangle(-100, -100, 100, 100))); // All
																					// points
	}
	...
	public void testContains() throws Exception {
		...
		assertTrue(circle1.contains(new Point(-Math.sqrt(2) / 2,
				Math.sqrt(2) / 2)));
	...
	public void testCloneDistanceCircle() throws Exception {
		...
		assertEquals(cd2.getRadius(), circle1.getRadius() - 0.5,
				Constant.EPSILON);
	}
