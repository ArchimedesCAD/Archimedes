...
public class TextTest extends Tester {
	...
	public void testClone() throws Exception {
		...
		Text offsetText = new Text("Test", new Point(10, 10), 10.0,
				new StubFont("", 10));
	...
	public void followsEqualsAndHashCodeContract() {
		...
		Text equalWithinEpsilonText = createSafeText(ARCHIMEDES, POINT,
				SIZE + 10e-10);
		...
		Text textWithOtherFont = createSafeText(ARCHIMEDES, new Point(0, 0),
				SIZE, new StubFont("", 1));
	...
	public void testClickInsideRectangleBoundaryText() {
		...
		Assert.assertTrue("Click not inside rectangle boundary text.",
				text.isInside(makeClickRectangle(clickPoint)));
	}
	...
	public void testClickOutsideRectangleBoundaryText() {
		...
		Point clickPoint = new Point(5, 5);
		Assert.assertFalse("Click not inside rectangle boundary text.",
				!text.isInside(makeClickRectangle(clickPoint)));
	...
	private Rectangle makeClickRectangle(Point clickPoint) {
		...
		double delta = br.org.archimedes.Utils.getWorkspace()
				.getSelectionSize() / 2.0;
		delta = br.org.archimedes.Utils.getWorkspace().screenToModel(delta);
		Point a = new Point(clickPoint.getX() - delta, clickPoint.getY()
				- delta);
		Point b = new Point(clickPoint.getX() + delta, clickPoint.getY()
				+ delta);
