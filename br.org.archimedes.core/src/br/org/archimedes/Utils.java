...
public class Utils {
	 ...
	 *            ortogonalize or breakAngles)
	 * @param point
	 *            The point that defines the vector
	 * @return If shift key is pressed, break the angles in sectors of 15
	 *         degrees If orto is on, the point that makes an ortogonal vector,
	 *         the same point otherwise.
	 * @throws NullArgumentException
	 *             Thrown if any argument is null.
	 */
	public static Point transformVector(Point p1, Point p2)
			throws NullArgumentException {

		Workspace workspace = getWorkspace();
		if (workspace.isShiftDown()) {
			p2 = Geometrics.breakAngles(p1, p2);
		} else if (workspace.isOrtoOn()
				&& workspace.getGripMousePosition() == null) {
	...
	public static IContextService getContextService() {
		...
		try {
			return (IContextService) PlatformUI.getWorkbench()
