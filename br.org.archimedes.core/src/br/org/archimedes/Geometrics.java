...
public class Geometrics {
	...
	public static double calculateDistance(Point p1, Point p2)
		...
		double returnValue = 0.0;
		if (p1 != null && p2 != null) {
			returnValue = calculateDistance(p1.getX(), p1.getY(), p2.getX(),
					p2.getY());
		} else {
			throw new NullArgumentException();
		}
	...
	public static double calculateDistance(Point startLine, Point endLine,
		...
		c = startLine.getX() * endLine.getY() - endLine.getX()
				* startLine.getY();
		double distance = Math.abs(point.getX() * a + point.getY() * b + c)
				/ Math.sqrt(a * a + b * b);
	...
	public static Vector breakAngles(Vector v) throws NullArgumentException {
		final double increment = Math.PI / 12.0;
		double referenceAngle = 0.0;
		double angle = calculateAngle(new Point(0, 0),
				new Point(v.getX(), v.getY()));
		Point point;
		...
		if (Math.abs(v.getX()) > Math.abs(v.getY())) {
			...
			if (referenceAngle <= Math.PI / 2.0) {
				...
				point = new Point(-v.getY() * Math.tan(referenceAngle),
						v.getY());
	 ...
	 * 
	 * if (isParallelTo(line1, line2)) { return null; }
	 * 
	 * Point intersectionPoint = null;
	 * 
	 * // The first line will be represented by a1x + b1y + c1 = 0 // The second
	 * line will be represented by a2x + b2y + c2 = 0 double a1, b1, c1; double
	 * a2, b2, c2;
	 * 
	 * Collection<Point> linePoints = line1.getExtremePoints(); Point
	 * initialPoint; Point endingPoint;
	 * 
	 * Collection<Point> secondLinePoints = line2.getExtremePoints(); Point
	 * secondInitialPoint; Point secondEndingPoint;
	 * 
	 * Iterator iterator = linePoints.iterator(); initialPoint = (Point)
	 * iterator.next(); endingPoint = (Point) iterator.next();
	 * 
	 * iterator = secondLinePoints.iterator(); secondInitialPoint = (Point)
	 * iterator.next(); secondEndingPoint = (Point) iterator.next();
	 * 
	 * a1 = initialPoint.getY() - endingPoint.getY(); b1 = endingPoint.getX() -
	 * initialPoint.getX(); c1 = (initialPoint.getX() - endingPoint.getX()) *
	 * initialPoint.getY() + (endingPoint.getY() - initialPoint.getY())
	 * initialPoint.getX();
	 * 
	 * a2 = secondInitialPoint.getY() - secondEndingPoint.getY(); b2 =
	 * secondEndingPoint.getX() - secondInitialPoint.getX(); c2 =
	 * (secondInitialPoint.getX() - secondEndingPoint.getX())
	 * secondInitialPoint.getY() + (secondEndingPoint.getY() -
	 * secondInitialPoint.getY()) secondInitialPoint.getX();
	 * 
	 * if ( !((Math.abs(a1 * b2 - b1 * a2) <= Constant.EPSILON) && (Math .abs(c1
	 * - c2) <= Constant.EPSILON))) {
	 * 
	 * double yIntersection = 0.0; double xIntersection = 0.0;
	 * 
	 * if (Math.abs(a1) <= Constant.EPSILON) { yIntersection = -(c1 / b1);
	 * xIntersection = -(b2 * yIntersection + c2) / a2; } else if (Math.abs(a2)
	 * <= Constant.EPSILON) { yIntersection = -(c2 / b2); xIntersection = -(b1 *
	 * yIntersection + c1) / a1; } else if (Math.abs(b1) <= Constant.EPSILON) {
	 * xIntersection = -(c1 / a1); yIntersection = -(a2 * xIntersection + c2) /
	 * b2; } else if (Math.abs(b2) <= Constant.EPSILON) { xIntersection = -(c2 /
	 * a2); yIntersection = -(a1 * xIntersection + c1) / b1; } else {
	 * yIntersection = ((c1 * a2) - (c2 * a1)) / ((b2 * a1) - (b1 * a2));
	 ...
	 * @return <tt>true</tt> if this infinite line is vertical within a certain
	 ...
	 * 
	 * double verInitialX, verInitialY, verEndingX, verEndingY; double n = 1.0 /
	 * Math.tan(line.getAngle()); verInitialX = n * (initialY -
	 * line.getInitialPoint().getY()) + line.getInitialPoint().getX();
	 * verEndingX = n * (endingY - line.getInitialPoint().getY()) +
	 * line.getInitialPoint().getX();
	 * 
	 * verInitialY = initialY; verEndingY = endingY;
	 * 
	 * if (verInitialX > verEndingX) { double temp = verInitialX; verInitialX =
	 * verEndingX; verEndingX = temp; temp = verInitialY; verInitialY =
	 * verEndingY; verEndingY = temp; }
	 * 
	 * Line verLine = new Line(verInitialX, verInitialY, verEndingX,
	 * verEndingY); return verLine; }
	 */
	 ...
	 * 
	 * double horInitialX, horInitialY, horEndingX, horEndingY; double m =
	 * Math.tan(line.getAngle()); horInitialY = m * (initialX -
	 * line.getInitialPoint().getX()) + line.getInitialPoint().getY();
	 * horEndingY = m * (endingX - line.getInitialPoint().getX()) +
	 * line.getInitialPoint().getY();
	 * 
	 * horInitialX = initialX; horEndingX = endingX; Line horLine = new
	 * Line(horInitialX, horInitialY, horEndingX, horEndingY); return horLine; }
	 */
	 ...
	 * 
	 * Line segment1 = element1.getSegment(); Line segment2 =
	 * element2.getSegment();
	 * 
	 * if (segment1 != null && segment2 != null) { double deltay1 =
	 * segment1.getInitialPoint().getY() - segment1.getEndingPoint().getY();
	 * double deltax1 = segment1.getInitialPoint().getX() -
	 * segment1.getEndingPoint().getX(); double deltay2 =
	 * segment2.getInitialPoint().getY() - segment2.getEndingPoint().getY();
	 * double deltax2 = segment2.getInitialPoint().getX() -
	 * segment2.getEndingPoint().getX();
	 * 
	 * if (Math.abs(deltax1) <= Constant.EPSILON && Math.abs(deltax2) <=
	 * Constant.EPSILON) { isParallel = true; } else { double m1 = deltay1 /
	 * deltax1; double m2 = deltay2 / deltax2; if (Math.abs(m1 - m2) <=
	 * Constant.EPSILON) { isParallel = true; } }
	 * 
	 * }
	 * 
	 * return isParallel; }
	 ...
	 */
	/*
	 * public static Line calculateBissector (Line line, Line otherLine) throws
	 * NullArgumentException {
	 * 
	 * if (line == null || otherLine == null) { throw new
	 * NullArgumentException(); }
	 * 
	 * Line bissector = null;
	 * 
	 * try { double distance = 1; Line lineCopy = (Line)
	 * line.cloneWithDistance(distance); Line otherLineCopy = (Line)
	 * otherLine.cloneWithDistance(distance);
	 * 
	 * Collection<Point> intersections = line
	 * .getIntersectionWithLine(otherLine); Point initialPoint = null; if
	 * (intersections.size() > 0) { initialPoint =
	 * intersections.iterator().next(); }
	 * 
	 * lineCopy = (Line) line.cloneWithDistance( -distance); otherLineCopy =
	 * (Line) otherLine.cloneWithDistance( -distance);
	 * 
	 * intersections = lineCopy.getIntersectionWithLine(otherLineCopy); Point
	 * endingPoint = null; if (intersections.size() > 0) { endingPoint =
	 * intersections.iterator().next(); }
	 * 
	 * boolean lineCopyContain = lineCopy.contains(endingPoint); boolean
	 * otherLineCopyContain = otherLineCopy.contains(endingPoint);
	 * 
	 * Vector direction = new Vector(initialPoint, endingPoint);
	 * 
	 * if ((lineCopyContain && !otherLineCopyContain) || ( !lineCopyContain &&
	 * otherLineCopyContain)) { direction = direction.getOrthogonalVector(); }
	 * 
	 * bissector = new Line(initialPoint, initialPoint .addVector(direction)); }
	 * catch (NullArgumentException e) { // Thrown if there are no intersections
	 * } catch (InvalidArgumentException e) { // Should not happen because the
	 * intersections between lines and // their copies will be diferent for sure
	 * e.printStackTrace(); }
	 * 
	 * return bissector; }
	 */
	 ...
	 * throws NullArgumentException {
	 * 
	 * if (line == null || point == null) { throw new NullArgumentException(); }
	 * 
	 * Vector direction = new Vector(line.getInitialPoint(), line
	 * .getEndingPoint()); direction = direction.getOrthogonalVector(); Line
	 * orthogonalLine = null; try { orthogonalLine = new Line(point,
	 * point.addVector(direction)); } catch (Exception e) { // Should never
	 * happen e.printStackTrace(); }
	 * 
