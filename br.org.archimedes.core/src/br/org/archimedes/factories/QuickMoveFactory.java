...
public class QuickMoveFactory extends TwoPointFactory {
	...
	private Map<Element, Collection<Point>> createClones(
		...
		for (Element element : pointsToMove.keySet()) {
			...
			clones.put(clone,
					extractedClonedPointsToMove(clone, originalPoints));
		}
		return clones;
	}
	...
	/**
	 * @param clone
	 *            The cloned element from which we want to obtain the points
	 *            from
	 * @param points
	 *            Points from the original element that we wish to move on the
	 *            clone
	 * @return A collection of points from the clone that we wish to move
	 */
	private Collection<Point> extractedClonedPointsToMove(Element clone,
		...
		for (Point point : points) {
			...
			if (pointIndex >= 0) {
				// If there are repeated points to move, I don't want to find
				// this one again
	...
	protected void drawVisualHelper(Point start, Point end) {
		...
		for (Entry<Element, Collection<Point>> association : movableClones
				.entrySet()) {
