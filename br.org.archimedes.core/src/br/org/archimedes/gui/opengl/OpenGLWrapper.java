...
public class OpenGLWrapper {
	 ...
	 */
	public void clear() {
		Color currentBkColor = br.org.archimedes.Utils.getWorkspace()
				.getBackgroundColor();
		GL11.glClearColor((float) currentBkColor.getR(),
				(float) currentBkColor.getG(), (float) currentBkColor.getB(), 0);
	...
	private CmapFormat getCmapFormat(Font font) {
		...
		if (format != null)
			...
			throw new IllegalStateException(Messages.bind(
					Messages.OpenGLWrapper_InvalidFont,
