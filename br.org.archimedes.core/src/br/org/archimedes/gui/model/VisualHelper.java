...
public class VisualHelper {
	 ...
	 * Constructor.
	 */
	public VisualHelper(OpenGLWrapper openGLWrapper, Workspace workspace,
	...
	public void draw(boolean cursorVisible) {
		...
		Color layerColor;
		try {
			layerColor = Utils.getController().getActiveDrawing()
	...
	private void drawCrossCursor(Point screenPoint) {
		...
		Point rightHorizontal = new Point(windowSize.getWidth() / 2,
				screenPoint.getY());
		Point topVertical = new Point(screenPoint.getX(),
				-windowSize.getHeight() / 2);
		Point bottomVertical = new Point(screenPoint.getX(),
				windowSize.getHeight() / 2);
	...
	private void drawOrientationArrows() {
		try {
			drawOrientationArrows(workspace.getOrientationArrowWidth(),
					workspace.getOrientationArrowLength(),
					workspace.getCursorColor());
		} catch (Exception e) {
		}
	}
	 ...
	 */
	private boolean drawOrientationArrows(double arrowWidth,
			double arrowLength, Color color) throws NullArgumentException {
		...
		opengl.draw(generateArrowPoints(arrowWidth, rightArrowInitialPoint,
				rightArrowFinalPoint));
		opengl.draw(generateArrowPoints(arrowWidth, upArrowInitialPoint,
				upArrowFinalPoint));
	...
	/*
	 * Generate the points used to draw an arrow from initialPoint to finalPoint
	 * with width arrowWidth Returns: list of points
	 */
	private List<Point> generateArrowPoints(double arrowWidth,
			Point initialPoint, Point finalPoint) throws NullArgumentException {

		double angle;
		try {
			angle = Math.atan((finalPoint.getY() - initialPoint.getY())
