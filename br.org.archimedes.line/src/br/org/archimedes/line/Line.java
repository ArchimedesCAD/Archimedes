...
public class Line extends Element implements Offsetable, Filletable {
	...
	private double getAngle() {
		...
		return Geometrics.calculateAngle(initialPoint.getX(),
				initialPoint.getY(), endingPoint.getX(), endingPoint.getY());
	}
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((this.endingPoint == null) ? 0 : this.endingPoint.hashCode());
		result = result
				+ ((this.initialPoint == null) ? 0 : this.initialPoint
						.hashCode());
		return result;
	}
	 ...
	 * 
	 * @see
	 * br.org.archimedes.model.Element#equals(br.org.archimedes.model.Element)
	 */
	 ...
	 * @return true if the initial point matches my initial and the ending
	 *         matches my ending, false otherwise
	 */
	private boolean pointsMatch(Point initial, Point ending) {

		return getInitialPoint().equals(initial)
				&& getEndingPoint().equals(ending);
	}
	...
	private double getLength() {
		...
		return Geometrics.calculateDistance(initialPoint.getX(),
				initialPoint.getY(), endingPoint.getX(), endingPoint.getY());
	}
	 ...
	 * 
	 * @see
	 * br.org.archimedes.model.Element#isInside(br.org.archimedes.model.Rectangle
	 * )
	 */
	...
	public Collection<UndoableCommand> getFilletCommands(Point arcCenter,
		...
		if (isInvalidFillet(arcCenter, arcIntersectionWithThisElement,
				arcIntersectionWithThatElement, pointToBeMoved)) {
			UndoableCommand eraseThis = new PutOrRemoveElementCommand(this,
					true);
			ret.add(eraseThis);
		} else {
			Collection<Point> points = new ArrayList<Point>();
			points.add(pointToBeMoved);
			HashMap<Element, Collection<Point>> hash = new HashMap<Element, Collection<Point>>();
			hash.put(this, points);
			try {
				if (force != null)
					ret.add(new MoveCommand(hash, new Vector(pointToBeMoved,
							force)));
				else
					ret.add(new MoveCommand(hash, new Vector(pointToBeMoved,
	...
	public Point getPointToBeMovedForFillet(Point arcCenter,
		...
		Point pointToBeMoved;
		boolean signAreaElement1 = Geometrics.calculateSignedTriangleArea(
				arcIntersectionWithThisElement, arcIntersectionWithThatElement,
				initialPoint) > 0;
		boolean signAreaElement2 = Geometrics.calculateSignedTriangleArea(
				arcIntersectionWithThisElement, arcIntersectionWithThatElement,
				endingPoint) > 0;

		if (signAreaElement1 == signAreaElement2) {
			if (Geometrics.calculateDistance(initialPoint,
					arcIntersectionWithThisElement) < Geometrics
					.calculateDistance(endingPoint,
							arcIntersectionWithThisElement))
				pointToBeMoved = initialPoint;
			else
				pointToBeMoved = endingPoint;
		} else {
			boolean signArea = Geometrics.calculateSignedTriangleArea(
					arcIntersectionWithThisElement,
					arcIntersectionWithThatElement, arcCenter) > 0;
			pointToBeMoved = (signArea == signAreaElement2) ? initialPoint
					: endingPoint;
		}
		return pointToBeMoved;
	}

	public boolean isInvalidFillet(Point arcCenter,
			Point arcIntersectionWithThisElement,
			Point arcIntersectionWithThatElement, Point pointToBeMoved) {
		Point otherPoint = (pointToBeMoved == initialPoint) ? endingPoint
				: initialPoint;
		boolean expectedSignArea = Geometrics.calculateSignedTriangleArea(
				arcIntersectionWithThisElement, arcIntersectionWithThatElement,
				arcCenter) > 0;
		boolean filletSignArea = Geometrics.calculateSignedTriangleArea(
				arcIntersectionWithThisElement, arcIntersectionWithThatElement,
				otherPoint) > 0;
	...
	public Point getTangencyLinePoint(Point intersection, Point click) {
		if (click.equals(intersection)) {
			return initialPoint.equals(intersection) ? endingPoint
					: initialPoint;
		}
		return click;
	}
