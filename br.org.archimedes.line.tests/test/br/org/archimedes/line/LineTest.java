...
public class LineTest extends Tester {
	...
	public void loadTestes() throws InvalidArgumentException {
		...
		lineFromPoint[0] = createSafeLineFromPoint(new Point(0, 0), new Point(
				10, 0));
		lineFromPoint[1] = createSafeLineFromPoint(new Point(0, 0), new Point(
				0, 10));
		lineFromPoint[2] = createSafeLineFromPoint(new Point(0, 0), new Point(
				10, 10));
		lineFromPoint[3] = createSafeLineFromPoint(new Point(0, 3), new Point(
				4, 0));
		lineFromPoint[4] = createSafeLineFromPoint(new Point(0, 3), new Point(
				-10, -15));
	...
	public void cantCreateALineFromEqualPointsPassingCoordinates()
			throws Exception {
	...
	@Test(expected = InvalidArgumentException.class)
	public void cantCreateALineFromEqualPointsPassingZeroPoints()
			throws Exception {
		new Line(new Point(0, 0), new Point(0, 0));
	}

	@Test(expected = InvalidArgumentException.class)
	public void cantCreateALineFromEqualPointsPassingNegativePoints()
			throws Exception {
		new Line(new Point(-1, -1), new Point(-1, -1));
	}

	@Test(expected = InvalidArgumentException.class)
	public void cantCreateALineFromEqualPointsPassingPositivePoints()
			throws Exception {
	...
	@Test
	public void lineContainsPointsBetweenItsInitialAndEndingPoints()
			throws Exception {
	...
	public void clonesWithDistanceMakingParallelLineToCorrectSideAndDistance() {
		...
		expected = createSafeLineFromDouble(-movement, movement, 1 - movement,
				1 + movement);
		copyLine = (Line) line.cloneWithDistance(0.5);
		Assert.assertEquals("The lines should be the same.", expected, copyLine);
		...
		line = createSafeLineFromDouble(0.0, 0.0, -1.0, 1.0);
		expected = createSafeLineFromDouble(-movement, -movement,
	...
	public void cloningKeepsSameLayer() throws Exception {
		...
		Layer layer = new Layer(new Color(0, 200, 20), "layer",
	...
	public void testEquals() {
		...
		Assert.assertFalse("These lines should be equal.",
				line1.equals(new Object()));
		...
		Assert.assertFalse("These lines should not be equal.",
				line1.equals(line4));
	}
	...
	// TODO Test the reference points of a line only consider the points withint
	// the rectangle
	 ...
	 */
	private Line createSafeLineFromDouble(double x1, double y1, double x2,
			double y2) {
	...
	@Test
	public void testPointToBeMovedForFilletWithDifferentAreaSign()
			throws InvalidArgumentException, NullArgumentException {
		Line line = new Line(0, 0, 2, 0);
		Point movedPoint = line.getPointToBeMovedForFillet(new Point(1, 1),
				new Point(1, 0), new Point(1.5, 1));
		Point expectedMovedPoint = line.getEndingPoint();
		assertEquals(expectedMovedPoint, movedPoint);

		movedPoint = line.getPointToBeMovedForFillet(new Point(1, -1),
				new Point(1, 0), new Point(1.5, -1));
		expectedMovedPoint = line.getEndingPoint();
		assertEquals(expectedMovedPoint, movedPoint);

		movedPoint = line.getPointToBeMovedForFillet(new Point(1, 1),
				new Point(1, 0), new Point(.5, 1));
	...
	@Test
	public void testPointToBeMovedForFilletWithEqualAreaSign()
			throws InvalidArgumentException, NullArgumentException {
		Line line = new Line(0, 0, 2, 0);
		Point movedPoint = line.getPointToBeMovedForFillet(new Point(2.5, 1),
				new Point(2.5, 0), new Point(3, 1));
		Point expectedMovedPoint = line.getEndingPoint();
		assertEquals(expectedMovedPoint, movedPoint);

		movedPoint = line.getPointToBeMovedForFillet(new Point(2.5, -1),
				new Point(2.5, 0), new Point(3, -1));
		expectedMovedPoint = line.getEndingPoint();
		assertEquals(expectedMovedPoint, movedPoint);

		movedPoint = line.getPointToBeMovedForFillet(new Point(-0.5, 1),
	...
	public void testTangencyLinePoint() throws InvalidArgumentException {
		...
		assertEquals(line.getInitialPoint(),
				line.getTangencyLinePoint(point3, point3));
		assertEquals(line.getEndingPoint(),
				line.getTangencyLinePoint(point2, point2));
	}
