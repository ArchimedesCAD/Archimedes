...
public class Semiline extends Element implements Offsetable, Filletable {
	...
	private double getAngle() {
		...
		return Geometrics.calculateAngle(initialPoint.getX(),
				initialPoint.getY(), directionPoint.getX(),
				directionPoint.getY());
	}
	...
	public Rectangle getBoundaryRectangle() {
		...
		return new Rectangle(getInitialPoint().getX(),
				getInitialPoint().getY(), getDirectionPoint().getX(),
				getDirectionPoint().getY());
	}
	...
	public Point getProjectionOf(Point point) throws NullArgumentException {
		...
		double projectionLength = direction.dotProduct(new Vector(initialPoint,
				point));
	...
	public String toString() {
		...
		return Messages.bind(Messages.Semiline_toString,
				initialPoint.toString(), directionPoint.toString());
	}
	 ...
	 * @return An empty list, if this line does not cross the rectangle; a list
	 *         with one point, if the starting point is inside the rectangle; or
	 *         a list with two points, if the semiline crosses the rectangle
	 * @throws NullArgumentException
	 ...
	 *            is.
	 * @param rectangle
	 *            The rectangle that will decide whether the other point should
	 *            be added or not
	 * @param trueCasePoint
	...
	public Collection<UndoableCommand> getFilletCommands(Point arcCenter,
		...
		try {
			...
			Point candidateDirectionPoint2 = arcIntersectionWithThisElement
					.addVector(new Vector(candidateDirectionPoint1,
							arcIntersectionWithThisElement));
			Line line = new Line(candidateDirectionPoint1,
					candidateDirectionPoint2);

			Point pointToBeMoved = line.getPointToBeMovedForFillet(arcCenter,
					arcIntersectionWithThisElement,
					arcIntersectionWithThatElement);
			Point fixedPoint = (pointToBeMoved.equals(candidateDirectionPoint1)) ? candidateDirectionPoint2
					: candidateDirectionPoint1;

			Semiline producedSemiline;
			if (force == null)
				producedSemiline = new Semiline(arcIntersectionWithThisElement,
						fixedPoint);
			else
				producedSemiline = new Semiline(force, fixedPoint);

			if (producedSemiline.contains(initialPoint)) {
				Line producedLine = new Line(producedSemiline.initialPoint,
						initialPoint);
	...
	public Point getTangencyLinePoint(Point intersection, Point click) {
		if (click.equals(intersection)) {
			return initialPoint.equals(intersection) ? directionPoint
					: initialPoint;
		}
		return click;
	}
