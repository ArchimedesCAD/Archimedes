...
public class ExtendCommand implements UndoableCommand {
	 ...
	 *            HashMap containing the elements to be extended and theirs
	 *            respective points
	 */
	public ExtendCommand(Collection<Element> references,
	...
	private void computeExtend(Drawing drawing, Point click)
		...
		if (key == null || isInMap) {
			...
			if (isInMap) {
				...
				turnedTo = new HashSet<Element>(
						Collections.singleton(extendResult));
				key = toExtend;
			}
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((elementsToExtend == null) ? 0 : elementsToExtend.hashCode());
		result = prime * result
	...
	public boolean equals(Object obj) {
		...
		if (!(this.elementsToExtend.keySet().containsAll(
				other.elementsToExtend.keySet()) && other.elementsToExtend
		...
		for (Point point : this.elementsToExtend.keySet()) {
			...
			if (!this.elementsToExtend.get(point).equals(
					other.elementsToExtend.get(point))) {
				return false;
			}
