...
public class ExtendFactory implements CommandFactory {
	...
	private String makeUndo() {
		...
		if (count > 0) {
			...
			count--;
			returnMessage = "Last operation undone." + Constant.NEW_LINE
					+ Messages.ExtendSelectElements;
		} else if (gotRef) {
			references = null;
			gotRef = false;
			returnMessage = "Reseting references." + Constant.NEW_LINE
					+ Messages.SelectRefs;
		} else {
		...
		if (area != null) {
			...
			for (Element element : elements) {
				...
				if (!contains) {
					...
					try {
						// TODO Shouldn't need Polyline to get the intersections
						// in a rectangle
						intersections = intersectionManager
								.getIntersectionsBetween(element, new Polyline(
										area));
						...
						for (Point extreme : element.getExtremePoints()) {
							for (Point intersection : intersections) {
								distance = Geometrics.calculateDistance(
