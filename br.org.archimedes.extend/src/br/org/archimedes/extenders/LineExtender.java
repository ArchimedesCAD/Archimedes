...
public class LineExtender implements Extender {
	...
	private boolean doExtend(Line line, Point extremePoint,
		...
		for (Point intersection : intersectionPoints) {
			...
			double distanceToRef = Geometrics.calculateDistance(intersection,
					extremePoint);
		...
		if (nearestReferencePoint != null) {
			...
			if (extremePoint.equals(line.getEndingPoint())) {
				offset = new Vector(line.getEndingPoint(),
						nearestReferencePoint);
				line.getEndingPoint().move(offset.getX(), offset.getY());
			} else {
				offset = new Vector(line.getInitialPoint(),
						nearestReferencePoint);
	...
	public Collection<Element> getInfiniteExtensionElements(Element element) {
		...
		try {
			InfiniteLine infiniteLine = new InfiniteLine(
					line.getInitialPoint(), line.getEndingPoint());
