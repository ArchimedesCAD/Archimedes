...
public class Text extends Element {
	...
	public Text clone() {
		...
		Text clone = null;
		try {
			clone = new Text(new String(text), originPoint.clone(),
					getHeight(), getFont());
			applyRotation(clone);
	...
	public Collection<ReferencePoint> getReferencePoints(Rectangle area) {
		...
		try {
			ReferencePoint reference = new RhombusPoint(originPoint,
					getPoints());
	...
	/**
	 * 
	 * The behavior this method is a little different this common because when
	 * select a text it is necessary checked if the click rectangle boundary is
	 * inside the text rectangle boundary
	 * 
	 * */
	@Override
	public boolean isInside(Rectangle rectangle) {
		return super.isInside(rectangle)
				|| rectangle.isInside(getBoundaryRectangle());
	}
