...
public class ArcTrimTest extends Tester {
	...
	public void setUp() throws NullArgumentException, InvalidArgumentException {
		...
		testArc = new Arc(new Point(1.0, 0.0), new Point(0.0, 1.0), new Point(
				-1.0, 0.0));
		trimmer = new ArcTrimmer();
	}

	@Test
	public void trimWithPointInTheMiddleResultsHalfArc()
			throws NullArgumentException, InvalidArgumentException {

		Collection<Element> trimmed = trimmer
				.trim(testArc, Collections.singleton(new Point(0.0, 1.0)),
						new Point(1.0, 0.0));
		assertCollectionTheSame(Collections.singleton(new Arc(new Point(0.0,
				1.0), new Point(-1.0, 0.0), new Point(0.0, 0.0), true)),
				trimmed);
	}
	...
	public void intersectionPointsInExtremePointsOfArcReturnsSameArc()
		...
		points.add(new Point(1.0, 0.0));
		Collection<Element> trimmed = trimmer.trim(testArc, points, new Point(
				1.0, 0.0));
		assertCollectionTheSame(Collections.singleton(testArc), trimmed);
	}

	@Test
	public void emptyIntersectionPointListReturnsSameArc()
			throws NullArgumentException, InvalidArgumentException {

		List<Point> emptyList = Collections.emptyList();
		Collection<Element> trimmed = trimmer.trim(testArc, emptyList,
				new Point(1.0, 0.0));
		assertCollectionTheSame(Collections.singleton(testArc), trimmed);
	}
	...
	public void twoIntersectionPointsMustUseFirstIntersectionPointAndReturnThreeQuartersOfArc()
		...
		Collection<Element> trimmed = trimmer.trim(testArc, points, new Point(
				1.0, 0.0));
		assertCollectionTheSame(Collections.singleton(new Arc(
				firstIntersection, new Point(0.0, 1.0), new Point(-1.0, 0.0))),
				trimmed);
	}

	@Test
	public void intersectionPointInExtremeOfArcReturnsSameArc()
			throws Exception {

		Collection<Element> trimmed = trimmer.trim(testArc, Collections
				.singleton(new Point(-1.0, 0.0)), new Point(-1.0, 0.0));
		assertCollectionTheSame(Collections.singleton(testArc), trimmed);
	}
}
