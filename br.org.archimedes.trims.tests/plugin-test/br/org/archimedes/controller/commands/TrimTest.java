...
public class TrimTest extends Tester {
	...
	private void doTrim(Element element, Collection<Element> references,
		...
		try {
			...
			Assert.fail("Should not throw any exception but threw "
					+ e.getMessage());
		}
	}
	...
	public void testTrimArc() throws Exception {
		...
		references = new LinkedList<Element>();
		reference1 = createSafeInfiniteLine(-COS_45 * 100, 0, -COS_45 * 100,
				100);
		references.add(reference1);
		...
		references.add(reference1);
		reference2 = createSafeInfiniteLine(0, -COS_45 * 100, 100,
				-COS_45 * 100);
		references.add(reference2);
	 ...
	 */
	private Element createSafeInfiniteLine(double x1, double y1, double x2,
			double y2) throws InvalidArgumentException {
