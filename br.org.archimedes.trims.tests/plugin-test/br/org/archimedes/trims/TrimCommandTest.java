...
public class TrimCommandTest extends Tester {
	...
	@Test(expected = NullArgumentException.class)
	public void throwsNullArgumentExceptionIfReferencesIsNull()
			throws Exception {
	...
	public void throwsIllegalActionExceptionIfClickIsNotOnAElement()
			throws Exception {
	...
	public void throwsIllegalActionExceptionIfTrimCantBeDone() throws Exception {
		...
		when(
				intersectionManager.getIntersectionsBetween(eq(line3),
						argThat(isACollectionEquivalentTo(expectedReferences))))
				.thenReturn(intersections);

		// Pretends that the trim can't be done
		Collection<Element> trimResult = Collections.emptyList();
		when(trimManager.getTrimOf(line3, intersections, click)).thenReturn(
				trimResult);
	...
	@Test
	public void usesAllElementsAsReferencesIfNoReferenceIsGiven()
			throws Exception {
		...
		trimResult
				.add(new Semiline(new Point(50.0, 50.0), new Point(0.0, 50.0)));
		when(trimManager.getTrimOf(line3, intersections, click)).thenReturn(
				trimResult);
	...
	@Test
	public void generatedCommandPutsOnlyTheUniqueResultsOfTrim()
			throws Exception {
		...
		Collection<Point> result = intersectionManager.getIntersectionsBetween(
				poly, args);
		...
		when(trimManager.getTrimOf(polyline, intersections, firstClick))
				.thenReturn(trimResult);
		...
		when(trimManager.getTrimOf(firstResult, intersections, secondClick))
				.thenReturn(secondTrimResult);
		...
		when(trimManager.getTrimOf(secondResult, intersections, thirdClick))
				.thenReturn(thirdTrimResult);
	...
	public void testEquals() throws Exception {
		...
		TrimCommand allElementsWithClickInOrigin = new TrimCommand(
				drawing.getUnlockedContents(),
				Collections.singletonList(new Point(0.0, 0.0)));
		TrimCommand allElementsWithClickOutOfOrigin = new TrimCommand(
				drawing.getUnlockedContents(),
				Collections.singletonList(new Point(1.0, 1.0)));
		TrimCommand line1WithClickInOrigin = new TrimCommand(references,
				Collections.singletonList(new Point(0.0, 0.0)));
		TrimCommand line1WithClickOutOfOrigin = new TrimCommand(references,
				Collections.singletonList(new Point(1.0, 1.0)));
		TrimCommand otherAllElementsWithClickInOrigin = new TrimCommand(
				drawing.getUnlockedContents(),
				Collections.singletonList(new Point(0.0, 0.0)));

		assertFalse(allElementsWithClickInOrigin
				.equals(allElementsWithClickOutOfOrigin));
		assertFalse(allElementsWithClickInOrigin.equals(line1WithClickInOrigin));
		assertFalse(allElementsWithClickInOrigin
				.equals(line1WithClickOutOfOrigin));
		assertFalse(allElementsWithClickOutOfOrigin
				.equals(line1WithClickInOrigin));
		assertFalse(allElementsWithClickOutOfOrigin
				.equals(line1WithClickOutOfOrigin));
		assertFalse(line1WithClickInOrigin.equals(line1WithClickOutOfOrigin));
		assertEquals(allElementsWithClickInOrigin,
				otherAllElementsWithClickInOrigin);
		assertEquals(otherAllElementsWithClickInOrigin,
				allElementsWithClickInOrigin);
		assertEquals(allElementsWithClickOutOfOrigin,
				allElementsWithClickOutOfOrigin);
	}
	...
	public void hashCodesAreEqualWhenCommandsAreEqual() throws Exception {
		...
		TrimCommand otherAllElementsWithClickInOrigin = new TrimCommand(
				drawing.getUnlockedContents(),
				Collections.singletonList(new Point(0.0, 0.0)));
		assertEquals(allElementsWithClickInOrigin.hashCode(),
				otherAllElementsWithClickInOrigin.hashCode());
	}
