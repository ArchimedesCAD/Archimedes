...
/**
 * Utility class with methods to convert basic elements to XML such as: Point
 * and Vector<br>
 * Replaces the write(...) methods of the old version to export pieces of the
 * drawing that are not Elements. Should write the XML according to the
 * specification FileSchema.xsd
 * 
 * @author julien
 */
public final class XMLExporterHelper {
	...
	/**
	 * POINT_FORMAT Escreve a representação XML deste ponto neste OutputStream
	 * usando este tag name.
	 * 
	 * @param point
	 *            o Point
	 * @param tag
	...
	/**
	 * Escreve a representação XML deste Vector neste OutputStream. POINT_FORMAT
	 * 
	 * @param vector
	 *            o Vector
	 * @param output
	...
	public static String xmlFor(final Point point) {
		...
		return String.format(POINT_FORMAT, POINT_TAG, point.getX(),
				point.getY());
	}
	...
	public static String xmlFor(final Vector vector) {
		...
		return String.format(VECTOR_FORMAT, VECTOR_TAG,
				xmlFor(POINT_TAG, vector.getPoint()), VECTOR_TAG);
	}

	public static String xmlFor(final String tag, final Vector vector) {

		return String.format(VECTOR_FORMAT, tag,
				xmlFor(POINT_TAG, vector.getPoint()), tag);
	}
