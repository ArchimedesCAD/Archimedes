/*
 * Copyright 2001 The Apache Software Foundation Licensed under the Apache
...
public class PostTable implements Table {
	...
	protected PostTable(DirectoryEntry de, RandomAccessFile raf)
		...
		version = raf.readInt();
		int italicAngle = raf.readInt();
		short underlinePosition = raf.readShort();
		short underlineThickness = raf.readShort();
		int isFixedPitch = raf.readInt();
		int minMemType42 = raf.readInt();
		int maxMemType42 = raf.readInt();
		int minMemType1 = raf.readInt();
		int maxMemType1 = raf.readInt();
	...
	public String getGlyphName(int i) {
		...
		if (version == 0x00020000) {
			return (glyphNameIndex[i] > 257) ? psGlyphName[glyphNameIndex[i] - 258]
