...
public class GlyfTable implements Table {
	...
	protected GlyfTable(DirectoryEntry de, RandomAccessFile raf)
			throws IOException {
		 ...
		 * descript = new TableGlyfDescript[t_maxp.getNumGlyphs()]; for (int i =
		 * 0; i < t_maxp.getNumGlyphs(); i++) { raf.seek(tde.getOffset() +
		 * t_loca.getOffset(i)); int len = t_loca.getOffset((short)(i + 1)) -
		 * t_loca.getOffset(i); if (len > 0) { short numberOfContours =
		 * raf.readShort(); if (numberOfContours < 0) { // descript[i] = new
		 * TableGlyfCompositeDescript(this, raf); } else { descript[i] = new
		 * TableGlyfSimpleDescript(this, numberOfContours, raf); } } else {
		 * descript[i] = null; } }
		 * 
		 * for (int i = 0; i < t_maxp.getNumGlyphs(); i++) {
		 * raf.seek(tde.getOffset() + t_loca.getOffset(i)); int len =
		 * t_loca.getOffset((short)(i + 1)) - t_loca.getOffset(i); if (len > 0)
		 * { short numberOfContours = raf.readShort(); if (numberOfContours < 0)
		...
		for (int i = 0; i < numGlyphs; i++) {
			...
			if (len > 0) {
				...
				if (numberOfContours >= 0) {
					descript[i] = new GlyfSimpleDescript(this,
							numberOfContours, bais);
				}
			} else {
				descript[i] = null;
			}
