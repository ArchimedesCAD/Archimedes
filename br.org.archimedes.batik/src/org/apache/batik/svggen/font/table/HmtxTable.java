...
public class HmtxTable implements Table {
	...
	protected HmtxTable(DirectoryEntry de, RandomAccessFile raf)
			throws IOException {
		 ...
		 * TableHhea t_hhea = (TableHhea) td.getEntryByTag(hhea).getTable(); int
		 * lsbCount = t_maxp.getNumGlyphs() - t_hhea.getNumberOfHMetrics();
		 * hMetrics = new int[t_hhea.getNumberOfHMetrics()]; for (int i = 0; i <
		 * t_hhea.getNumberOfHMetrics(); i++) { hMetrics[i] = raf.readInt(); }
		 * if (lsbCount > 0) { leftSideBearing = new short[lsbCount]; for (int i
		 * = 0; i < lsbCount; i++) { leftSideBearing[i] = raf.readShort(); } }
		 */
	}
