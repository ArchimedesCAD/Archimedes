...
public class DXFImporter implements Importer {
	...
	private void parseBlocksFrom(DXFLayer dxfLayer) {
		Iterator blockIterator = kabejaParser.getDocument()
				.getDXFBlockIterator();
	...
	private Layer addParsedElementsFrom(DXFLayer dxfLayer) {
		...
		Layer archLayer = new Layer(layerBackgroundColor, dxfLayer.getName(),
				LineStyle.CONTINUOUS, dxfLayer.getLineWeight());
		archLayer.setPrintColor(new Color(0, 0, 0));
		Collection<ElementParser> parsers = ElementParser.getParserMap()
				.values();
			...
			try {
				for (Element element : parser.parse(dxfLayer)) {
					try {
						...
						System.out
								.println("Elemento parseado já está na Layer e não será inserido novamente.");
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
