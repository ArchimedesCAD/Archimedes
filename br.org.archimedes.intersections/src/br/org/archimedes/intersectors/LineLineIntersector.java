...
public class LineLineIntersector implements Intersector {
	...
	/*
	 * If the lines are parallels, including cases of same lines or containing
	 * lines, this method returns a empty LinkedList
	 */
	public Collection<Point> getIntersections(Element element,
		...
		if (isParallelTo(firstLine, secondLine)) {
			if (firstLine.getEndingPoint().equals(secondLine.getInitialPoint())
					&& !(secondLine.contains(firstLine.getInitialPoint()) || firstLine
				...
				intersectionPoints.add(firstLine.getEndingPoint());
					...
					&& !(secondLine.contains(firstLine.getEndingPoint()) || firstLine
		...
		if (!((Math.abs(a1 * b2 - b1 * a2) <= Constant.EPSILON) && (Math.abs(c1
			...
			if (firstLine.contains(intersection)
					&& secondLine.contains(intersection))
