...
public class EllipseTest {
	...
	public void shouldNotCreateEllipseIfPointsAreTooCloseOfCenter()
			throws Exception {
		new Ellipse(center1, center1, heightPoint1);
	}

	@Test(expected = InvalidArgumentException.class)
	public void shouldNotCreateEllipseIfColinearPointsArePassed()
			throws Exception {
	...
	public void shouldGetEqualSemiMajorAxis() {
		assertEquals(ellipse1.getSemiMajorAxis(), new Vector(center1,
				widthPoint1));
	...
	public void testHashCodeEllipse() throws Exception {
		...
		assertEquals(hash1, ellipse1.hashCode()); // Test if it returns the same
													// code again
		assertNotSame(hash1, hash2);

		Ellipse e1 = new Ellipse(ellipse1.getCenter(),
				ellipse1.getWidthPoint(), ellipse1.getHeightPoint());
		int hash_e1 = e1.hashCode();
		assertEquals(hash1, hash_e1);
		e1.move(5, 0);
		assertNotSame(hash_e1, e1.hashCode()); // Object changed, hash should
												// change
	}
	...
	@Test
	public void shouldPositiveDirectionBeTrueWhenPointIsOutside()
			throws Exception {
	...
	@Test
	public void shouldContainsCenterAndAllFourIntersectionsWithAxis()
			throws Exception {
		...
		Collection<Point> points = Arrays.asList(center1, heightPoint1,
				widthPoint1, oppositeHeightPoint, oppositeWidthPoint);

		Collection<? extends ReferencePoint> referencePoints = ellipse1
				.getReferencePoints(container);
	...
	public void shouldCalculateOffset() throws Exception {
		Ellipse expectedEllipse = new Ellipse(center1, new Point(0, 15),
				new Point(-10, 0));
	...
	public void shouldReturnProjectionOfPointAtEllipse() throws Exception {
		Point expectedPoint = new Point(5 * Math.sqrt(2.0) / 2.0,
				10 * Math.sqrt(2.0) / 2.0);
	...
	public void shouldCalculateFocusPointsOfAnEllipse() {
		...
		Point expectedPoint1 = center1
				.addVector(expectedE1.multiply(expectedF));
		Point expectedPoint2 = center1.addVector(expectedE1
				.multiply(-expectedF));
