...
public class PolylineTrimmer implements Trimmer {
	...
	public Collection<Element> trim(Element element,
		...
		if (halfSegmentBeforeClick != null) {
			nearestNegativeIntersectionPoint = getNearestIntersectionPoint(
					cutPoints, halfSegmentBeforeClick, click);
		...
		if (nearestNegativeIntersectionPoint == null) {
			if (!polyline.isClosed()) {
				for (negativeIntersectedIndex = clickSegmentIndex - 1; negativeIntersectedIndex >= 0; negativeIntersectedIndex--) {
					...
					nearestNegativeIntersectionPoint = getNearestIntersectionPoint(
							cutPoints, line, line.getEndingPoint());
			...
			// se o poligono for fechado
				...
				int cont = 0;
				// procura por intersecoes em sentido anti-horario, passando
				// inclusive pela origem e para os segmentos anteriores ate
				// olhar todo poligono
				for (negativeIntersectedIndex = clickSegmentIndex - 1; cont < numberOfLines; negativeIntersectedIndex--, cont++) {
					negativeIntersectedIndex = mod(negativeIntersectedIndex,
							numberOfLines);
					Line line = lines.get(negativeIntersectedIndex);
		...
		if (halfSegmentAfterClick != null) {
			nearestPositiveIntersectionPoint = getNearestIntersectionPoint(
					cutPoints, halfSegmentAfterClick, click);
		...
		if (nearestPositiveIntersectionPoint == null) {
			if (!polyline.isClosed()) {
				for (positiveIntersectedIndex = clickSegmentIndex + 1; positiveIntersectedIndex < lines
					...
					nearestPositiveIntersectionPoint = getNearestIntersectionPoint(
							cutPoints, line, line.getInitialPoint());
				...
				int numberOfLines = lines.size();
				int cont = 0;
				// procura por intersecoes em sentido horario, passando
				// inclusive pela origem e continuando para os proximos
				// segmentos ate olhar todo poligono
				for (positiveIntersectedIndex = clickSegmentIndex + 1; cont < numberOfLines; positiveIntersectedIndex++, cont++) {
					positiveIntersectedIndex = mod(positiveIntersectedIndex,
							numberOfLines);
					Line line = lines.get(positiveIntersectedIndex);
		...
		// constroi o poligono a ser devolvido, com o trim passando pelos
		// segmentos que fecham o poligono
		if (negativeIntersectedIndex > clickSegmentIndex) {
			List<Point> points = new ArrayList<Point>();
			points.add(nearestPositiveIntersectionPoint);
			points.addAll(polyPoints.subList(positiveIntersectedIndex + 1,
		...
		// segmentos que fecham o poligono
			...
			if (trimResult.size() == 2) {
				...
				if (lastPoly1.equals(firstPoly2)) {
					...
					try {
						double determinant = Geometrics.calculateDeterminant(
								last, firstPoly2, poly2Points.get(1));
	...
	/**
	 * Get the nearest intersection point from a given reference that is inside
	 * line.
	 * 
	 * @param cutPoints
	 ...
	 * @return The point in cutPoints that is closest to reference and is inside
	 *         line.
	 * @throws NullArgumentException
	 ...
	 * @author lreal
	 */
	private Point getNearestIntersectionPoint(Collection<Point> cutPoints,
			Line line, Point reference) throws NullArgumentException {
