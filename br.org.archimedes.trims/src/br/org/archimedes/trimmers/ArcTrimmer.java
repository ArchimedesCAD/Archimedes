...
public class ArcTrimmer implements Trimmer {
	...
	public Collection<Element> trim(Element element,
		...
		try {
			...
			initial = new ComparablePoint(arc.getInitialPoint(), new DoubleKey(
					0.0));
		...
		try {
			...
			// The intersection is exactly at one extreme of the arc; does
			// nothing
			trimResult.clear();
			trimResult.add(arc);
		}
	...
	public SortedSet<ComparablePoint> getSortedPointSet(Arc arc,
			Point referencePoint, Collection<Point> points) {
