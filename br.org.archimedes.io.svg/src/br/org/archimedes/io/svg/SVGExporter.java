...
public class SVGExporter implements Exporter {
	 ...
	 * @param charset
	 *            The charset to use to write the file @ * @throws IOException
	 *            Thrown if something goes wrong when writing the file
	 * @throws UnsupportedEncodingException
	 *             Thrown if the system cannot write in the specified charset
	 */
	private void exportSVGHeader(Drawing drawing, OutputStream output,
			String charset, Rectangle boundaryRectangle) throws IOException,
			UnsupportedEncodingException {

		StringBuilder drawingTag = new StringBuilder(
				"<?xml version=\"1.0\" encoding=\"" + charset //$NON-NLS-1$
						+ "\"?>" + "\n"); //$NON-NLS-1$ //$NON-NLS-2$

		drawingTag
				.append("<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\"" //$NON-NLS-1$
				// + " width="10cm" height="3cm"

						+ " viewBox=\"" + (int) Math.ceil(boundaryRectangle.getLowerLeft().getX()) + " " //$NON-NLS-1$ //$NON-NLS-2$
						+ (int) Math.ceil(boundaryRectangle.getLowerLeft()
								.getY())
						+ " " //$NON-NLS-1$
	...
	private void writeLayersDefs(Collection<Layer> layers, OutputStream output,
