...
public class HorizontalInfiniteLineTest extends InfiniteLineTestCase {
	 ...
	 */
	private void testInfiniteLineCase(double x1, double y1, double x2, double y2)
			throws Exception {
	...
	private void testContains(double x, double y) {
		...
		try {
			Assert.assertTrue("The point " + point.toString()
					+ "does not belong to the infinite line",
					line.contains(point));
	...
	public void testCopy() throws Exception {
		 ...
		 * InfiniteLine(initialPoint, direction); copiedXLine =
		 * safeCloneWithDistance(xLine, 0.5); Assert.assertEquals(
		 * "The copied xLine and the original should be the same.", expected,
		 * copiedXLine);
		 */
		 ...
		 * Vector(new Point( -1.0, 1.0)); expected = new
		 * InfiniteLine(initialPoint, direction); copiedXLine =
		 * safeCloneWithDistance(xLine, 0.5); Assert.assertEquals("The copied
		 * xLine and the original should be the same.", expected, copiedXLine);
		 * initialPoint = new Point(0.5 / sqr2, 0.5 / sqr2); expected = new
	...
	public void testRotate() throws Exception {
		...
		Assert.assertEquals("point should be equal.",
				expected.getInitialPoint(), new Point(0, Math.sqrt(2)));
		Assert.assertEquals("point should be equal.",
				expected.getEndingPoint(),
