...
public class Polyline extends Element implements Offsetable, Filletable {
	...
	public Polyline(Point... points) throws NullArgumentException,
			InvalidArgumentException {
	 ...
	 */
	public Polyline(Rectangle rectangle) throws NullArgumentException,
			InvalidArgumentException {
	...
	private ArrayList<Point> getCrossings(Line ray)
			throws NullArgumentException {
	...
	public boolean isPositiveDirection(Point point) {
		...
		if (closestLines.size() == 1) {
			...
			Point crossing = getCrossing(closestLines.get(0),
					closestLines.get(1));
	...
	public Element cloneWithDistance(double distance)
		...
		List<InfiniteLine> offsetedSegments = getOffsettedLines(distance,
				segments);

		Point startingPoint = offsetedSegments.get(0).getInitialPoint();
		Point endingPoint = offsetedSegments.get(segments.size() - 1)
				.getEndingPoint();

		List<Point> points = extractLimits(offsetedSegments, startingPoint,
				endingPoint);
	 ...
	 * @return
	 */
	private List<Point> extractLimits(List<InfiniteLine> offsetedSegments,
		...
		if (this.isClosed()) {
			Collection<Point> intersectionsBetween = getIntersectionsBetween(
					offsetedSegments, 0, offsetedSegments.size() - 1);
	 ...
	 * @return A collection with the intersections between those segments of an
	 *         empty collection
	 */
	private Collection<Point> getIntersectionsBetween(
			List<InfiniteLine> offsetedSegments, int currentSegment,
			int nextSegment) {
	 ...
	 */
	private List<InfiniteLine> getOffsettedLines(double distance,
			List<Line> segments) {

		List<InfiniteLine> offsetedSegments = new ArrayList<InfiniteLine>();

		ExtendManager extendManager = new ExtendManagerEPLoader()
				.getExtendManager();
		for (int i = 0; i < segments.size(); i++) {
			Line line = (Line) segments.get(i).cloneWithDistance(distance);
			InfiniteLine extendedLine = (InfiniteLine) extendManager
	 ...
	 * 
	 * @see
	 * br.org.archimedes.model.PointSortable#getSortedPointSet(br.org.archimedes
	 * .model.Point, java.util.Collection)
	 */
	 ...
	 *         segment contains the intersection three things can happen: <LI>
	 *         return 0 if the intersection is in the extension of the first
	 *         segment <LI>return the index of the last segment if the
	 *         intersection is in the extension of the last segment <LI>the
	 *         number of segments (which is an invalid index).
	 * @throws NullArgumentException
	...
	public Collection<UndoableCommand> getFilletCommands(Point arcCenter,
		...
		Line intersectionLine = getLines().get(
				getNearestSegment(arcIntersectionWithThisElement));
		Point pointToBeMoved = intersectionLine.getPointToBeMovedForFillet(
				arcCenter, arcIntersectionWithThisElement,
				arcIntersectionWithThatElement);

		Collection<UndoableCommand> ret = new ArrayList<UndoableCommand>();
		if (!(intersectionLine.isInvalidFillet(arcCenter,
				arcIntersectionWithThisElement, arcIntersectionWithThatElement,
				pointToBeMoved))) {
			Point movedPoint = (force != null) ? force
					: arcIntersectionWithThisElement;

			Polyline newPolyline = null;

			try {
				if (pointToBeMoved == intersectionLine.getInitialPoint()) {
					newPolyline = new Polyline(getPointsAfter(pointToBeMoved,
							movedPoint));

				} else {
					newPolyline = new Polyline(getPointsBefore(pointToBeMoved,
							movedPoint));
	...
	public Point getTangencyLinePoint(Point intersection, Point click) {
		...
		try {
			if (nearestSegment.contains(click)) {
				...
				return nearestSegment.getTangencyLinePoint(intersection,
						intersection);
			}
		} catch (NullArgumentException e) {
			// Should not reach here
