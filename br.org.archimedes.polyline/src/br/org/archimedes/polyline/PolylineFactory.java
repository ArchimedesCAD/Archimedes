...
public class PolylineFactory implements CommandFactory {
	...
	private String getNextIterationMessage() {
		...
		default:
			return Messages.CreatePolyLineIteration4 + Messages.closeInitial;
		}
	}
	...
	public Parser getNextParser() {
		...
		if (active) {
			if (getPointsSize() == 0) {
				...
				Parser decoratedParser = new VectorParser(
						points.get(getPointsSize() - 1), false);
				parser = new StringDecoratorParser(decoratedParser,
						Messages.undoInitial);
			} else if (getPointsSize() == 2) {
				Parser vectorParser = new VectorParser(
						points.get(getPointsSize() - 1));
